...
def start_docker():
    """Start Synapse."""
    print("[Proxion] Starting Synapse Stack...")
    # Matrix requires a config generation step usually, but we assume it's pre-configured or handled by the user.
    subprocess.run(["docker-compose", "up", "-d"], check=True)

def stop_docker():
    print("[Proxion] Stopping Synapse Stack...")
...
def main():
    ...
    try:
        ...
        print("\n[Proxion] Matrix (Synapse) is RUNNING at http://localhost:8008")
        print(f"Proxion Messaging: {MOUNT_POINT} (Pod) -> /data (Docker)")
