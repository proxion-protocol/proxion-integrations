...
def start_docker():
    """Start FreshRSS."""
    print("[Proxion] Starting FreshRSS...")
    subprocess.run(["docker-compose", "up", "-d"], check=True)

def stop_docker():
    print("[Proxion] Stopping FreshRSS...")
...
def main():
    ...
    try:
        ...
        print("\n[Proxion] FreshRSS is RUNNING at http://localhost:8084")
        print(f"Proxion News: {MOUNT_POINT} (Pod) -> /data (Docker)")
