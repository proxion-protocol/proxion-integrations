...
def start_docker():
    """Start Pi-hole."""
    print("[Proxion] Starting Pi-hole...")
    subprocess.run(["docker-compose", "up", "-d"], check=True)

def stop_docker():
    print("[Proxion] Stopping Pi-hole...")
...
def main():
    ...
    try:
        ...
        print("\n[Proxion] Pi-hole is RUNNING at http://localhost:8088")
        print(f"Proxion DNS Guard: {MOUNT_POINT} (Pod) -> /etc/pihole (Docker)")
