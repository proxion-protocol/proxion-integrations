...
# Drive V: for Firefly III Data/Uploads
MOUNT_POINT = "P:" 
...
def start_docker():
    """Start Firefly III containers."""
    print("[Proxion] Starting Firefly III...")
    subprocess.run(["docker-compose", "up", "-d"], check=True)

def stop_docker():
    print("[Proxion] Stopping Firefly III...")
    subprocess.run(["docker-compose", "down"], check=True)

def main():
    mount_process = None
    # Ensure local dirs exist
    for d in ["db_data", "upload"]:
    ...
    try:
        ...
        print("\n[Proxion] Firefly III is RUNNING at http://localhost:8080")
        print(f"Finance Logic: Firefly (Docker) -> /var/www/html/storage/upload -> {MOUNT_POINT} (FUSE) -> Pod")
