...
def start_docker():
    """Start Mastodon."""
    print("[Proxion] Starting Mastodon Stack...")
    # Ensure .env.production exists or use defaults
    if not os.path.exists(".env.production"):
        with open(".env.production", "w") as f:
            f.write("LOCAL_DOMAIN=localhost\nSECRET_KEY_BASE=dummy\nOTP_SECRET=dummy\n")
            
    subprocess.run(["docker-compose", "up", "-d"], check=True)

def stop_docker():
    print("[Proxion] Stopping Mastodon Stack...")
...
def main():
    ...
    try:
        ...
        print("\n[Proxion] Mastodon is RUNNING at http://localhost:3000")
        print(f"Proxion Social: {MOUNT_POINT} (Pod) -> /system, /postgres, /redis (Docker)")
